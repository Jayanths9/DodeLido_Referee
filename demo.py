...
def image_input(model, classifier, frame):
    ...
    if circles is not None:
        ...
        for circle in filtered_circles:
            ...
            #  Predict the class of cropped card image
            class_names, confidence_scores = model_lite_predict(
                interpreter=interpreter, image=roi, classifier=mlb)
            ...
            for j, (class_name, confidence) in enumerate(zip(
                    top_two_classes, top_two_confidences)):
                if confidence > 0.5:
                    text_class1 = f"{class_name}: {confidence:.2f}"
                    text_class2 = f"{class_name}"
                    text_color1 = f"BGR: ({int(bgr_color[0])}, " \
                                  f"{int(bgr_color[1])}, {int(bgr_color[2])})"
                    cv2.putText(frame, text_class1, (x1,
                                                     y1 - y_offset * (j + 1)),
...
def camera_input(model, classifier, cap):
    ...
    if circles is not None:
        ...
        for circle in filtered_circles:
            ...
            class_names, confidence_scores = model_lite_predict(
                interpreter=interpreter, image=new_image, classifier=mlb)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run a specific function with arguments.")
    subparsers = parser.add_subparsers(dest="function",
                                       help="Function to run")

    # Subparser for camera
    parser_camera = subparsers.add_parser("camera",
                                          help="Run with camera input")
    parser_camera.add_argument("--model", type=str,
                               default="./resources/trained_model.tflite",
                               help="Path to trained model for image function")
    parser_camera.add_argument("--classifier", type=str,
                               default="./resources/classifier.pkl",
                               help="Path to classifier for image function")

    # Subparser for image
    parser_image = subparsers.add_parser("image", help="Run on local image")
    parser_image.add_argument("--model", type=str,
                              default="./resources/trained_model.tflite",
                              help="Path to trained model for image function")
    parser_image.add_argument("--classifier", type=str,
                              default="./resources/classifier.pkl",
                              help="Path to classifier for image function")
    parser_image.add_argument("--imagepath", type=str,
                              default="./resources/trialImage3.jpg",
                              help="Path to image for image function")
